...
class ScannerAgent(Agent):
    """
    ScannerAgent does 2 jobs:
    1) Get fresh deals from RSS feeds
    2) Ask OpenAI to pick the best 5 deals in a structured format
    """
...
"""
    ...
    def __init__(
        ...
        - If caller does not provide RSS/OpenAI objects, create defaults.
    ...
    def fetch_deals(self, memory: list[str] | None = None) -> list[ScrapedDeal]:
        """
        Fetch deals from RSS, skipping URLs already seen in memory.
        memory = list of URLs (strings).
        """
        known_urls = set(memory or [])
        self.log(f"Fetching RSS deals (known URLs: {len(known_urls)})")
    ...
    def make_user_prompt(self, scraped: list[ScrapedDeal]) -> str:
        """
        Build the user prompt from scraped deals.
        """
        deals_text = "\n\n".join(deal.describe() for deal in scraped)
        return f"{self.USER_PROMPT_PREFIX}{deals_text}{self.USER_PROMPT_SUFFIX}"

    def scan(self, memory: list[str] | None = None) -> DealSelection | None:
        """
        Main method:
        - fetch deals
        - send to OpenAI Structured Outputs
        ...
        parsed: DealSelection | None = completion.choices[0].message.parsed
        if parsed is None:
            self.log("OpenAI returned no parsed content")
            return None

        # Keep only deals with meaningful price
    ...
    def test_scan(self) -> DealSelection:
        """
        Offline test data for downstream pipeline testing.
        """
        results = {
            "deals": [
                {
